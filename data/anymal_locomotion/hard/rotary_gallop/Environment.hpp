...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
    ...
    // auto robotArtVisual_ = server_->addVisualArticulatedSystem("v_robot", resourceDir_+"/anymal/urdf/anymal.urdf");
    ...
    if (visualizable_) {
      ...
      // robotVisual_[0]->setColor({0.4f, 0.4f, 0.4f});

      // for (int i = 0; i < 4; i++) 
      // {
      //   auto foot0 = vis->getSceneManager()->getEntity(robotVisual_->at(20 + 3 * i).graphics->getAttachedObject(0)->getName());
      //   foot0->setMaterialName("steel");
      //   // foot0->setColor({0.4f, 0.4f, 0.4f});
      //   // std::cout << robotVisual_->at(20+i*3).graphics->getAttachedObject(0)->getName() << std::endl;
      // }


      // std::cout << robot_->getVisOb() << std::endl;
      //  for (int i = 0; i < 4; i++) {
      //   auto foot0 = vis->getSceneManager()->getEntity(
      //       robotVisual_->at(29 + 10 * i).graphics->getAttachedObject(0)->getName());
      //   auto foot1 = vis->getSceneManager()->getEntity(
      //       robotVisual_->at(30 + 10 * i).graphics->getAttachedObject(0)->getName());
...
double gaitGenerationCost()
{
    ...
    // std::string mode = "half_bound";
    // std::string mode = "pace";
    std::string mode = "rotary_gallop";
...
inline void updateVisual() 
{
    ...
    // robot_->setColor({0,0,1,1});

    // for (int i = 0; i < 4; i++) {
    //   if (footContactState_[i]) {
    //     robot_.visual()[5 + 5 * i]->setColor({0.3f, 0.3f, 1.0f});
    //   } else {
    //     robot_.visual()[5 + 5 * i]->setColor({0.0f, 0.0f, 0.0f});
    //   }
  // }
// }
...
*/
  ...
  bool isTerminalState(float& terminalReward) final {
    ...
    if ((gc_[0] < -0.3) || (abs(gc_[1]) > 1.0)) // Координаты
    {
      if(visualizeThisStep_) 
